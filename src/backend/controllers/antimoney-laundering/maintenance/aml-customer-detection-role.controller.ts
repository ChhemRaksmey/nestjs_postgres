...
export class ControllerAMLMaintenanceCustomerDetectionRole {
  ...
  async save_edit(
    ...
    try {
      ...
      return res.redirect(`/aml/maintenance/customer-detection-role?success=${success_message}`);
    } catch (err: any) {
      return res.render('anti-money-laundering/maintenance/customer-detection-role/edit', { 
        user: (req.session as any).user,
        error: err.message, 
      });
    }
